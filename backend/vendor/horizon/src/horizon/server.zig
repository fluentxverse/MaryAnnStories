...
pub const Server = struct {
    ...
    fn handleConnection(self: *Self, connection: net.Server.Connection) !void {
        ...
        var http_server = http.Server.init(connection, read_buffer[0..]);
        ...
        while (http_server.state == .ready) {
            ...
            const head_bytes = http_server.read_buffer[0..request.head_end];
            var header_iter = std.mem.splitSequence(u8, head_bytes, "\r\n");
            ...
            // Send response
            var extra_headers: std.ArrayListUnmanaged(http.Header) = .{};
    ...
    fn sendStreamingResponse(
        ...
        var body_writer = request.respondStreaming(.{
            .send_buffer = body_writer_buffer[0..],
        ...
        switch (streaming_body) {
            .file => |file_stream| {
                ...
                while (true) {
                    ...
                    try writer.writeAll(chunk_buffer[0..read_bytes]);
                }
            },
        }
